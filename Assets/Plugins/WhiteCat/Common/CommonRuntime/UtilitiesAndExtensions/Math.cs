...
namespace WhiteCat
{
	...
	public partial struct Math
	{
		...
		/// 线性映射
		/// </summary>
		public static float LinearMapping(float input, float inputMin, float inputMax, float outputMin, float outputMax)
		{
			return (outputMax - outputMin) * Mathf.Clamp01((input - inputMin) / (inputMax - inputMin)) + outputMin;
		}
